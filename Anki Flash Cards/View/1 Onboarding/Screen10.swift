...
struct TenScreen: View {
    ...
    var body: some View {
        ...
        .onAppear {
            startTime = Date()
            Analytics.logEvent("ten_screen_appear", parameters: nil)
            
            // Логируем старт анимации прогресса
            Analytics.logEvent("ten_screen_progress_animation_start", parameters: nil)
            ...
            _ = Timer.scheduledTimer(withTimeInterval: 0.03, repeats: true) { timer in
                withAnimation(.linear(duration: 0.03)) {
                    if progress < 1.0 {
                        ...
                        timer.invalidate()
                        Analytics.logEvent("ten_screen_progress_animation_end", parameters: nil)
                    }
                }
            }
            
            // Последовательное появление галочек с логами
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                withAnimation(.spring()) {
                    showChecks[0] = true
                    Analytics.logEvent("ten_screen_check_1_appear", parameters: nil)
                }
            }
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
                withAnimation(.spring()) {
                    showChecks[1] = true
                    Analytics.logEvent("ten_screen_check_2_appear", parameters: nil)
                }
            }
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.5) {
                withAnimation(.spring()) {
                    showChecks[2] = true
                    Analytics.logEvent("ten_screen_check_3_appear", parameters: nil)
                }
            }
        ...
        .onDisappear {
            Analytics.logEvent("ten_screen_disappear", parameters: nil)
            if let start = startTime {
                let duration = Date().timeIntervalSince(start)
                Analytics.logEvent("ten_screen_time_spent", parameters: [
